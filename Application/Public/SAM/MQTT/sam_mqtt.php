...
define("SAM_MQTT_ID", "SAM_MQTT_ID");
define("SAM_MQTT_USER", "SAM_MQTT_USER");
define("SAM_MQTT_PASS", "SAM_MQTT_PASS");
/* ---------------------------------
    SAMConnection
   --------------------------------- */
...
class SAMConnection_MQTT {
  ...
  */
  ...
  function Connect($proto, $options=array()) {
	...
	if( array_key_exists(SAM_MQTT_ID, $options)){
		$this->sub_id = $options[SAM_MQTT_ID];
	}
	
	if( array_key_exists(SAM_MQTT_USER, $options)){
		$this->username = $options[SAM_MQTT_USER];
	}
	if( array_key_exists(SAM_MQTT_PASS, $options)){
		$this->password = $options[SAM_MQTT_PASS];
	}
  ...
  function do_connect_now() {
	...
	$payload1 = $this->utf($this->username);
	$payload2 = $this->utf($this->password);

    /* add in the remaining length field and fix it together   */
    $msg = $this->fixed_header("MQTT_CONNECT") . $this->remaining_length(strlen($variable)+strlen($payload)+strlen($payload1)+strlen($payload2)) . $variable . $payload . $payload1 . $payload2;
